			...
			if(grad[0]) {
				for(int cp1=0; cp1<nrcomp; cp1++) {
					for(int np=1; np<=npars[cp1]; np++) {
						...
						models.psitfinal[ng*nrcomp+cp1](np) += npsum/sf1;
						if(grInd[0]) gradIndi(nr,np+1) = npsum/sf1;
					}
					int prst=0;
					double mpsum=0.0;
					for(int cp2=0; cp2<nrcomp; cp2++) {
					...
					mptfinal[ng](cp1+1) += mpsum/sf1;
					if(grInd[0]) gradIndi(nr,1) = mpsum/sf1;
				}
			}
...
void ngDataSetUp(int *groupnr, double *data, int *vars, int *modes, int *indReal, int *lengths, int *xmiss, int *print) {
	printlevel=print[0];
	int gnr=groupnr[0];
	int xm=xmiss[0];
	if(gnr<1||gnr>ngdat.getNGroups()) error("Wrong groupnr in data set up");
	if(printlevel>19) Rprintf("Setting data for group %d \n", gnr);
	ngdat.data[gnr-1].mtsdata(data,vars[0],modes,indReal[0],lengths,xm);
...
void ngCovSetUp(int *groupnr, double *data, int *vars, int *modes, int *indReal, int *lengths, int *xmiss, int *print) {
	printlevel=print[0];
	int gnr=groupnr[0];
	int xm=xmiss[0];
	if(gnr<1||gnr>ngcov.getNGroups()) error("Wrong groupnr in covariates set up");
	if(printlevel>19) Rprintf("Setting data for group %d \n", gnr);
	ngcov.data[gnr-1].mtsdata(data,vars[0],modes,indReal[0],lengths,xm);
